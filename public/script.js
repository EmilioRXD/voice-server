...
class VoiceDetector {
  ...
  async init() {
    // Nota: El AudioContext se crea bajo demanda cuando el usuario hace clic en "Join"
...
class MinecraftIntegration {
  ...
  processUpdate() {
    ...
    if (remoteDeafenedNow !== this.remoteDeafened) {
      ...
      console.log(
        `🔇 Remote deafen changed: ${this.remoteDeafened ? "DEAFENED" : "UNDEAFENED"
        }`
      );
  ...
  updateParticipantsList(participants) {
    ...
    participants.forEach((p) => {
      ...
      div.innerHTML = `
        ...
        <span class="participant-name">${info.gamertag}${info.isSelf ? " (You)" : ""
        }${distanceText}</span>
        ${!info.isSelf
...
class VoiceChatApp {
  ...
  setupPushToTalk() {
    ...
    this.ui.elements.pttToggle.addEventListener("change", (e) => {
      ...
      if (this.ws && this.ws.readyState === 1) {
        ...
        console.log(
          `📡 PTT toggle changed: ${enabled ? "ENABLED (muted)" : "DISABLED (talking)"
          }`
        );
      }
    });
  ...
  async connectToRoom() {
    ...
    try {
      // VALIDACIÓN: Verificar si estamos en file://
      if (window.location.protocol === "file:") {
        const msg = "⚠️ Error: EnviroVoice cannot connect to WebSockets when opened as a local file (file://).\n\n" +
          "Please use a local web server (like npx serve, Live Server, etc.) to run this application.";
        console.error(msg);
        alert(msg);
        this.ui.updateRoomInfo("❌ Scheme file:// not allowed for WebSockets");
        return;
      }
      ...
      if (micStream) {
        this.voiceDetector = new VoiceDetector(null, (isTalking, volumeDb) => {
          if (this.ws && this.ws.readyState === 1) {
            this.ws.send(
              JSON.stringify({
      ...
      // NORMALIZACIÓN DE URL: Asegurar que tenga protocolo y sea ws/wss
      let wsUrl = url.trim();

      // Si no tiene protocolo, decidimos si usar ws o wss
      if (!wsUrl.includes("://")) {
        const isLocal = wsUrl.includes("localhost") || wsUrl.includes("127.0.0.1");
        wsUrl = isLocal ? "ws://" + wsUrl : "wss://" + wsUrl;
      } else {
        // Si tiene protocolo, asegurar que sea ws/wss
        wsUrl = wsUrl.replace("http://", "ws://").replace("https://", "wss://");

        // REGLA ESPECIAL: Forzar wss para servicios conocidos como Render/onrender.com
        if (wsUrl.includes(".onrender.com") && wsUrl.startsWith("ws://")) {
          console.log("🔒 Forcing secure WebSocket (wss) for Render domain");
      ...
      this.ws.onclose = (event) => {
        ...
        if (!event.wasClean) {
          console.warn("⚠️ WebSocket connection was lost unexpectedly");
        }
        this.exitCall();
      };
    } catch (e) {
      ...
      if (data.muteStates) {
        ...
        if (myState) {
          ...
          if (myState.isMuted !== this.minecraft.remoteMuted) {
            ...
            console.log(
              `🎮 Minecraft mute changed: ${myState.isMuted ? "MUTED" : "UNMUTED"
              }`
            );
          ...
          if (myState.isDeafened !== this.minecraft.remoteDeafened) {
            ...
            console.log(
              `🔇 Minecraft deafen changed: ${myState.isDeafened ? "DEAFENED" : "UNDEAFENED"
              }`
            );
